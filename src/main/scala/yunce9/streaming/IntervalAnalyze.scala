...
object IntervalAnalyze {
  ...
  def getAllData(real_search: Array[ReceiveKafka]): (Dataset[TaskInfo], Array[SearchData]) = {
    ...
    val search_data = spark.sql(getSearch).persist()
    val searchArr = search_data.as[SearchData].collect()

    val matchdf: Dataset[MatchPresent] = getInfoData(search_str, 10, 37).where(s"present_json !='' and present_json!='null' and present_json is not null")
      ...
      .flatMap(m => {
        val gson = new Gson()
        val presents: Array[Present] = if (m.present_json != "null") gson.fromJson(m.present_json, classOf[Array[Present]]) else null
        val mainBody: Array[String] = if (m.mainBody_json != "null") m.mainBody_json.replaceAll("[\\[\\]]", "").split(" ") else null
        try {
          real_search.filter(f => f.searchId.toLong == m.searchID).map(p => MatchPresent(p.realId.toLong, m.nickName, m.id, m.author, m.content_words, m.platform, m.publish_date, m.risk_value, m.label, m.appraise, m.heat, getInfoTitle(m.title, m.nickName), m.topic_words, mainBody, presents))
        } catch {
          case _: Exception =>
            println(s"${NowDate()} ERROR 异常数据id: " + real_ids.toList)
            Array(MatchPresent(real_ids.head.toLong, m.nickName, m.id, m.author, m.content_words, m.platform, m.publish_date, m.risk_value, "", m.appraise, m.heat, getInfoTitle(m.title, m.nickName), m.topic_words, mainBody, presents))
        }
      })
    val getInfo = matchdf.withColumnRenamed("searchId", "real_id")
      .drop("task_time").join(search_data, Seq("real_id"))
    val infoDF: Dataset[TaskInfo] = getInfo.as[TaskInfo].persist()
    search_data.unpersist()
    (infoDF, searchArr)
  }
  ...
  def getMatchs(infoDF: Seq[TaskInfo]): Seq[Match] = {
    val matchData = infoDF.flatMap(all => {
      val name = all.nick_name.split(" ")(1)
